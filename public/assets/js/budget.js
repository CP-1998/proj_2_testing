...
((UIController, ExpenseController) => {
  ...
  document.getElementById('saveItem').addEventListener('click', event => {
    ...
    let name = expenseTypeMenu
    let description = document.getElementById('description').value
    let price = document.getElementById('expenseValue').value
    let userId = localStorage.getItem('userId')
    console.log(name, description, price, userId)
    axios.get('/api/items', { name, description, price, userId })
      .then(({ data: items }) => {
        console.log(items)
